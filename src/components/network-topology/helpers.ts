...
const generateTooltip = (data: any): string => {
    ...
    <div class="bg-white p-4 rounded shadow-lg max-w-sm">
      ...
      <p class="mb-1"><span class="font-semibold">IP:</span> ${data.ip}</p>
      <p class="mb-1"><span class="font-semibold">Type:</span> ${data.type}</p>
...
// Function to create a group template for network segments
const createGroupTemplate = () => {
    return $(
        go.Group,
        "Auto",
        {
            layout: $(go.ForceDirectedLayout, { defaultSpringLength: 50, defaultElectricalCharge: 100 }),
            background: "transparent",
        },
        $(go.Shape, "Rectangle", {
            fill: colorPalette.segment,
            stroke: "gray",
            strokeWidth: 2,
            opacity: 0.2,
        }),
    ...
    doLayout(coll: go.Diagram | go.Group | go.Iterable<go.Part>) {
        ...
        diagram.nodes.each((node) => {
            if (node.data.type === "router") {
                ...
                others.add(node)
                const subnet = node.data.ip.split(".").slice(0, 3).join(".")
...
// Remove the createVirtualizedModel function as we're not using it anymore
